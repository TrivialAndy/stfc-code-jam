...
def parse_input(filename: str) -> list:
    ...
    N, A, B: 
        N: Number of nodes
        A: Amadea paints every A-th node
        B: Bilva paints every B-th node
    P: N-1 values where P_i is the parent of node N_(i+1)

    TASK: 
    returns: list[(N, A, B), list[P]]
    """
    with open(filename) as f:
        num_test_cases = int(f.readline())
        test_cases = []
        for _ in range(num_test_cases):
            N, A, B = f.readline().strip().split(" ")
            test_cases.append([(N, A, B), [int(x) for x in f.readline().strip().split(" ")]])
    return test_cases
...
def write_output(filename: str, values: list) -> None:
    ...
    values list: List of expected beauty values. Solution from calculate_result()
    """
